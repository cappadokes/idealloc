...
impl Job {
    ...
    #[inline]
    pub fn place(&self, offset: usize, squeezed: bool) {
...
impl JobSet {
    ...
    fn t2(
        ...
        for (_space_id, mut set) in ris {
            ...
            let (boxed, unresolved) = set.lemma_1(h, e, rand_tamer);
        ...
        for jobs in igc_rows {
            ...
            if row_count % h == 0 {
                res.box_and_update(buf, h, rand_tamer);
    ...
    fn try_box(
        ...
        for (_, entry) in self.contents {
            if entry.is_original() {
                originals_boxed += 1;
                assert!(res.originals
                    .borrow_mut()
                    .insert(entry.clone()));
            } else {
                let mut to_enrich = res.originals.borrow_mut();
                entry.originals
                    .take()
                    .into_iter()
                    .for_each(|j| {
                        assert!(to_enrich.insert(j));
                    })
            }
            res._push(entry);
        }

        //rand_tamer.update_boxarr(res.originals.borrow());
    ...
    fn lemma_1(
        ...
        while self.contents.len() > 0 {
            ...
            // of each strip.
            res.strip_boxin(vertical, h, true, rand_tamer);
            res.strip_boxin(horizontal, h, false, rand_tamer);
        }
    ...
    fn strip_boxin(
        ...
        for (k, j) in trg.into_iter() {
            buffer.insert(k, j);
            if buffer.len() == h {
                self.box_and_update(buffer, h, rand_tamer);
    ...
    fn box_and_update(
        ...
        let (new_box, next_id, to_box) = helper.try_box(h, rand_tamer);
