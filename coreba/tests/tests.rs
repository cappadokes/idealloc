...
fn read_from_path<T, B>(p: &str, shift: ByteSteps) -> Result<JobSet, Box<dyn std::error::Error>> 
where T: JobGen<B> {
...
#[test]
fn run_hello_plc() {
    let set = read_from_path::<PLCParser, &[u8; 8 * PLC_FIELDS_NUM]>("tests/data/ldb0.plc", 0).unwrap();
