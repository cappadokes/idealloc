...
fn malloc(size: size_t) -> *mut void {
    ...
    if must_log() {
        let (map, block_size) = crate::mapfind::find_mapping(ptr);
        update_log(0, &[size, ptr as usize, block_size, pthread_self() as usize, map]);
    }
...
fn free(p: *mut void) {
    ...
    if must_log() {
        update_log(1, &[p as usize, pthread_self() as usize]);
    }
}
...
fn calloc(nobj: size_t, size: size_t) -> *mut void {
    ...
    if must_log() {
        let (map, block_size) = crate::mapfind::find_mapping(ptr);
        update_log(2, &[nobj, size, ptr as usize, block_size, pthread_self() as usize, map]);
    }
...
fn realloc(p: *mut void, size: size_t)  -> *mut void {
    ...
    if must_log() {
        let (map, block_size) = crate::mapfind::find_mapping(ptr);
        update_log(3, &[p as usize, size, ptr as usize, block_size, pthread_self() as usize, map]);
    }
...
fn aligned_alloc(alignment: size_t, size: size_t)  -> *mut void {
    ...
    if must_log() {
        let (map, block_size) = crate::mapfind::find_mapping(ptr);
        update_log(4, &[alignment, size, ptr as usize, block_size, pthread_self() as usize, map]);
    }
...
fn memalign(alignment: size_t, size: size_t)  -> *mut void {
    ...
    if must_log() {
        let (map, block_size) = crate::mapfind::find_mapping(ptr);
        update_log(5, &[alignment, size, ptr as usize, block_size, pthread_self() as usize, map]);
    }
...
fn posix_memalign(memptr: *mut *mut void, alignment: size_t, size: size_t)  -> int {
    ...
    if must_log() {
        let (map, block_size) = crate::mapfind::find_mapping(*memptr);
        update_log(6, &[memptr as usize, alignment, size, *memptr as usize, block_size, pthread_self() as usize, map]);
    }
